...
#define SCHED_MAX_EVENT_DATA_SIZE       10     /**< Maximum size of scheduler events. Note that scheduler BLE stack events do not contain any data, as the events are being pulled from the stack in the event handler. */
...
#define APP_SCHED_EVENT_HEADER_SIZE sizeof(event_header_t)       /**< Size of app_scheduler.event_header_t (only for use inside APP_SCHED_BUF_SIZE()). */
